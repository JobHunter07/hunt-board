...
- **SearchFilterBar**
  ...
  - **Responsive**: Single horizontal row; controls group does not wrap
...
- **KanbanBoardPage**
  - **Header Layout**: Single `Box` with `display: flex`, `justifyContent: space-between`, `alignItems: center` containing the title `Typography` (left) and `SearchFilterBar` (right)
...
- **SC-005**: All three header controls (Search, Filter, Add Target) remain fully functional after the layout refactor — filtering, searching, and modal opening all work correctly
